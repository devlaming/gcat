...
def CalcLogL(param,y1,y2,y1notnan,y2notnan,ybothnotnan,n,nboth,N,X,K,mode=3):
    ...
    if halt:
        ...
        if mode==1:
            ...
            return logL,None,None,None,None
        elif mode==5:
            return None
        return None,None
    # calculate errors 
    ...
    for i in range(TOTALCOLS):
        for j in range(i,TOTALCOLS):
            ...
            if j>i:
                ...
                H[:,j,:,i]=H[:,i,:,j]
    if mode==3: # if interested in logL, gradient, Hessian, return that
        return logL,grad,H
...
def Newton(param,y1,y2,y1notnan,y2notnan,ybothnotnan,n,nboth,N,X,K,silent=False,linesearch=True,snpmodel=False,quickinitialisation=True):
    ...
    while not(converged) and i<MAXITER:
        ...
        if snpmodel and i==0 and quickinitialisation:
            ...
            # calculate log-likelihood, gradient, Hessian, and individual-specific weights for grad and Hessian
            (logL,grad,H,wG,wH)=CalcLogL(param,y1,y2,y1notnan,y2notnan,ybothnotnan,n,nboth,N,X,K,mode=4)
...
def GCAT():
    ...
    global param0,logL0,grad0,H0,D0,converged0,wG0,wH0
    # print update
